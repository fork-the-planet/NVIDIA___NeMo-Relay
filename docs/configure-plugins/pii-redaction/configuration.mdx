...
- The real provider response value remains unchanged.
- The plugin sanitizes emitted NeMo Relay start and end event payloads.
- The plugin populates `annotated_response` from the sanitized end-event payload
  when a response codec is available.
